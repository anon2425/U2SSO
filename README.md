...
We introduce the concept of **Unlinkable User-issued Single-Sign-On (U2SSO)**, where users create their own Single-Sign-On master identities ($\Phi$), replacing the IdP, and store them in a distributed immutable Identity Registry (IdR). When signing up for a service, the user generates an unlinkable *child public key* from its master identity $\Phi$ and proves in zero-knowledge that it owns one of the $\Phi$ in IdR. The SP then uses this derived public key for future logins. 

U2SSO ensures unlinkability in a weak trust model such that even if the IdR and SPs collude, the unlinkability of service public keys is preserved. Additionally, U2SSO provides Sybil resistance with zero-knowledge proofs, allowing SPs to enforce rules to limit the creation of derived identities from the same $\Phi$ to only one account.
...
We provide two constructions: SNARK-U2SSO, and DBPoE-U2SSO in `./crypto` and `./crypto-ddh` directories.
